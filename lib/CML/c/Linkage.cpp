...
const Error *Linkage::StartMove( void )
{
   ...
   // Wait for drives in the linkage to clear abort bit - 10ms should be plenty of time
...
const Error *Linkage::SetControlWord( uint16 value )
{
   ...
   else
   {
      ...
      const Error *err = ecat->WaitCycleUpdate( 2 * ecat->GetCyclicPeriod() );
      if( err )
