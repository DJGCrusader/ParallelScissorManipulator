...
*/
/***************************************************************************/
const Error *Thread::stop( Timeout timeout )
{
   ...
   DWORD ret = WaitForSingleObject( tData->exitEvent, (int32)timeout );
...
*/
/***************************************************************************/
const Error *Thread::sleep( Timeout timeout )
{
   if( !timeout ) return 0;
...
*/
/***************************************************************************/
const Error *Semaphore::Get( Timeout timeout )
{
   if( !data ) return &ThreadError::Alloc;

   return WaitOnWindowsObject( (HANDLE)data, (int)timeout );
}
