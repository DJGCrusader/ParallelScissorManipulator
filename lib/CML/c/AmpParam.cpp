...
{
   int32 v;
   const Error *err = Upld32( OBJID_POS_LOAD, 0, v );
...
{
   int32 v;
   const Error *err = Upld32( OBJID_POS_MTR, 0, v );
   value = PosMtr2User( v );
   return err;
}
...
/***************************************************************************/
/**
  Get the load encoder position.  For single encoder systems, this value is
  NOT USED.

  For dual encoder systems, this function returns the load encoder position
  and is identical to the value returned by Amp::GetPositionActual.  This is
  also the passive load position when the load encoder is configured to be 
  in passive mode.
  ...
  */
/***************************************************************************/
const Error *Amp::GetPositionLoad( uunit &value )
{
   int32 v;
   const Error *err = Upld32( OBJID_POS_LOAD_PASSIVE, 0, v );
   value = PosLoad2User( v );
   return err;
}
...
{
   return Dnld32( OBJID_POS_LOAD, 0, PosUser2Load(value) );
}
...
{
   return Dnld32( OBJID_POS_MTR, 0, PosUser2Mtr(value) );
}

/***************************************************************************/
/**
  Set the load encoder position.  On single encoder systems, this value is
  NOT USED.
  
  For dual encoder systems, this will set the load encoder position.  This is
  also the passive load position when the load encoder is configured to be 
  in passive mode.
  ...
  */
/***************************************************************************/
const Error *Amp::SetPositionLoad( uunit value )
{
   return Dnld32( OBJID_POS_LOAD_PASSIVE, 0, PosUser2Load(value) );
}
...
{
   int32 v;
   const Error *err = Upld32( OBJID_POS_CMD, 0, v );
   value = PosLoad2User( v );
   return err;
}
...
/***************************************************************************/
/**
  Get the position error.  This is the difference between the instantaneous 
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_POS_ERR, 0, v );
   value = PosLoad2User( v );
   return err;
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_VEL_MTR, 0, v ); 
   value = VelMtr2User( v );
   return err;
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_VEL_LOAD, 0, v ); 
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_VEL_CMD, 0, v ); 
   value = VelMtr2User( v );
   return err;
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_VEL_LIM, 0, v ); 
   value = VelMtr2User( v );
   return err;
}
...
{
   return Upld16( OBJID_CRNT_ACT, 0, value );
}
...
{
   return Upld16( OBJID_CRNT_CMD, 0, value );
}
...
{
   return Upld16( OBJID_CRNT_LIM, 0, value );
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_TRJ_VEL, 0, v ); 
   value = VelLoad2User( v );
   return err;
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_TRJ_ACC, 0, v ); 
...
{ 
   return Upld16( OBJID_PHASE_ANG, 0, value ); 
}
...
{ 
   return Upld16( OBJID_HALL_STATE, 0, value ); 
}
...
{
   return Upld16( OBJID_ANALOG_REF, 0, value );
}
...
{
   return Upld16( OBJID_HVREF, 0, value );
}
...
{
   return Upld16( OBJID_AMPTEMP, 0, value );
}
...
{
   const Error *err = Upld16( OBJID_AENC_SIN, 0, sin );
   if( !err ) err = Upld16( OBJID_AENC_COS, 0, cos );
   return err;
}
...
{
   const Error *err = Upld16( OBJID_CNRT_U, 0, u );
   if( !err ) err = Upld16( OBJID_CRNT_V, 0, v );
   return err;
}
...
/**
  ...
  @param name The name of the drive to be set
  @return A pointer to an error object, or NULL on success
  */
/***************************************************************************/
const Error *Amp::SetAmpName( char *name )
{
   return DnldString( OBJID_AMP_NAME, 0, name );
}

/***************************************************************************/
/**
  Get the amplifier name stored in the amplifiers flash
  @param name The name of the drive is returned here.  This buffer should
         be at least 50 bytes long to avoid overflow.
  @return A pointer to an error object, or NULL on success
  */
/***************************************************************************/
const Error *Amp::GetAmpName( char *name )
{
   int32 l;
   return UpldString( OBJID_AMP_NAME, 0, l=COPLEY_MAX_STRING, name );
}
...
{
   return Dnld16( OBJID_PWM_MODE, 0, (uint16)mode );
}
...
{
   uint16 m;
   const Error *err = Upld16( OBJID_PWM_MODE, 0, m );
   mode = (AMP_PWM_MODE)m;
   return err;
}
...
{
   return Dnld16( OBJID_PHASE_MODE, 0, (uint16)mode );
}
...
{
   uint16 m;
   const Error *err = Upld16( OBJID_PHASE_MODE, 0, m );
...
{
   if( viaSDO )
      return Upld16( OBJID_INPUTS, 0, value ); 
...
/**
  ...
  Note that only the lower 16 input pins are PDO mapped, so this function 
  will always use an SDO access to read the full set of inputs.  If the input
  of interest is one of the lower 16 pins, then the method Amp::GetInputs()
  is generally a much faster way to read the pin's state.
  ...
  */
/***************************************************************************/
const Error *Amp::GetInputs32( uint32 &value )
{
   return Upld32( OBJID_INPUTS32, 0, value ); 
}
...
{
   return Dnld16( OBJID_PULLUPS, 0, value );
}
...
{
   return Upld16( OBJID_PULLUPS, 0, value );
}

/***************************************************************************/
/**
 32 Bit version of SetIoPullup.  This is useful on drives that support 
 more then 16 pull up/down resistors.  Please see Amp::SetIoPullup() for more
 details.
 ...
 */
/***************************************************************************/
const Error *Amp::SetIoPullup32( int32 value )
{
   return Dnld32( OBJID_PULLUPS32, 0, value );
}

/***************************************************************************/
/**
 32 Bit version of GetIoPullup.  This is useful on drives that support 
 more then 16 pull up/down resistors.  Please see Amp::GetIoPullup() for more
 details.
 ...
 */
/***************************************************************************/
const Error *Amp::GetIoPullup32( int32 &value )
{
   return Upld32( OBJID_PULLUPS32, 0, value );
}
...
/**
  ...
  function.  This method configures the specified input to perform
  ...
  @param axis The axis to apply this input configuration to. This only applies
  to multi-axis drives. Default is 0 (first axis/axis A).
  @return A pointer to an error object, or NULL on success
  */
/***************************************************************************/
const Error *Amp::SetInputConfig( int8 pin, INPUT_PIN_CONFIG cfg, uint16 axis )
{
   // Pin numbering starts at 0.
   if( pin < 0 ) return &SDO_Error::Subindex;

   // Get the new input config word
   uint16 axisConfig = SetIOConfigAxis( (uint16)cfg, axis );

   // Set the input config on the specified axis. Increment the pin to match subindex.
   return Dnld16( OBJID_INPUT_CFG, pin+1, axisConfig );
}
...
const Error *Amp::GetInputConfig( int8 pin, INPUT_PIN_CONFIG &cfg )
{
   uint16 c;
   if( pin < 0 ) return &SDO_Error::Subindex;
   const Error *err = Upld16( OBJID_INPUT_CFG, pin+1, c );
...
/**
  ...
  available.
  @param cfg The input pin function will be returned in this variable.
  @param axis The axis number that this input pin is configured for
  @return A pointer to an error object, or NULL on success
  */
/***************************************************************************/
const Error *Amp::GetInputConfig( int8 pin, INPUT_PIN_CONFIG &cfg, uint16 &axis )
{
   uint16 c;
   if( pin < 0 ) return &SDO_Error::Subindex;
   const Error *err = Upld16( OBJID_INPUT_CFG, pin+1, c );
   axis = GetIOConfigAxis( c );
   cfg = (INPUT_PIN_CONFIG)c;
   return err;
}
...
{
   if( pin < 0 ) return &SDO_Error::Subindex;
   return Dnld16( OBJID_DEBOUNCE, pin+1, value );
}
...
{
   if( pin < 0 ) return &SDO_Error::Subindex;
   return Upld16( OBJID_DEBOUNCE, pin+1, value );
}
...
/**
  ...
  @param axis    On multi-axis drives this is used to configure which axis this output pin
                 is configured for.

  @return A pointer to an error object, or NULL on success
  */
/***************************************************************************/
const Error *Amp::SetOutputConfig( int8 pin, OUTPUT_PIN_CONFIG cfg, uint32 param1, uint32 param2, uint16 axis )
{
   if( pin < 0 ) return &SDO_Error::Subindex;

   uint16 c = SetIOConfigAxis ( cfg, axis );
   byte buff[10];
   ...
   if( !CheckFeature( FEATURE_EXTENDED_OUTPUT_PIN_CONFIG ) )
      return Download( OBJID_OUTPUT_CFG, pin+1, 6, buff );

   buff[6] = ByteCast(param2);
   buff[7] = ByteCast(param2>>8);
   buff[8] = ByteCast(param2>>16);
   buff[9] = ByteCast(param2>>24);
   return Download( OBJID_OUTPUT_CFG, pin+1, 10, buff );
}
...
const Error *Amp::GetOutputConfig( int8 pin, OUTPUT_PIN_CONFIG &cfg, uint32 &param1, uint32 &param2 )
{
   ...
   const Error *err = Upload( OBJID_OUTPUT_CFG, pin+1, size, buff );
...
/**
  ...
  @param param1 The pin's first 32-bit parameter will be returned here
  @param param2 The pin's second 32-bit parameter will be returned here
  @param axis   On multi-axis drives the axis number that this pin is configured 
                for will be returned here
  @return A pointer to an error object, or NULL on success
  */
/***************************************************************************/
const Error *Amp::GetOutputConfig( int8 pin, OUTPUT_PIN_CONFIG &cfg, uint32 &param1, uint32 &param2, uint16 &axis )
{
   if( pin < 0 ) return &SDO_Error::Subindex;
...
/**
  ...
  @param cfg  The pin configuration value will be returned here.
  @param mask The pin's status bit selection mask will be returned here.
  @param axis On multi-axis drives the axis number that this output pin is
              configured for will be returned here
  @return     A pointer to an error object, or NULL on success
  */
/***************************************************************************/
const Error *Amp::GetOutputConfig( int8 pin, OUTPUT_PIN_CONFIG &cfg, uint32 &mask, uint16 &axis )
{
   uint32 mask2;
   return GetOutputConfig( pin, cfg, mask, mask2, axis );
}
...
/**
  ...
  */
/***************************************************************************/
const Error *Amp::GetOutputConfig( int8 pin, OUTPUT_PIN_CONFIG &cfg, uint16 &axis )
{
   uint32 mask1, mask2;
   return GetOutputConfig( pin, cfg, mask1, mask2, axis );
}
...
{
   return Dnld16( OBJID_OUTPUTS, 0, value ); 
}
...
{
   return Upld16( OBJID_OUTPUTS, 0, value ); 
}
...
/**
  ...
  This parameter is used to configure the optional features of the general
  purpose IO. Bits 0-3 describe whether several IO pins are used as a serial
  interface for expanded IO features. 0 is normal IO, 1 is AEM/APM development
  board LEDs and address switches. 2 is LEDs wired the same as the developers
  kit board, but using separate red & green LEDs for network status.

  @param value The new value for the IOOptions configuration
  @return A pointer to an error object, or NULL on success
  */
/***************************************************************************/
const Error *Amp::SetIOOptions( int32 value )
{
   return Dnld32( OBJID_IO_OPTIONS, 0, value );
}
...
/**
  ...
  */
/***************************************************************************/
const Error *Amp::GetIOOPtions( int32 &value )
{
   return Upld32( OBJID_IO_OPTIONS, 0, value );
}
...
{
   uint32 v;
   const Error *err = Upld32( OBJID_FAULTS, 0, v );
   value = (AMP_FAULT)v;
   return err;
}
...
/**
  ...
  */
/***************************************************************************/
const Error *Amp::ClearFaults( void )
{
   // Clear latched faults using the control word
   const Error *err;
   if( lastCtrlWord & 0x0080 )
   {
      err = SetControlWord( lastCtrlWord & ~0x0080 );
      if( err ) return err;
   }
   ...
   return SetControlWord( lastCtrlWord & ~0x0080 );
}
...
{
   return Dnld32( OBJID_FAULT_MASK, 0, (uint32)value );
}
...
{
   uint32 v;
   const Error *err = Upld32( OBJID_FAULT_MASK, 0, v );
   value = (AMP_FAULT)v;
   return err;
}
...
const Error *Amp::GetEventStatus( EVENT_STATUS &stat )
{
   ...
   uint16 eventStatusObj = (axisNum) ? 0x2185 : OBJID_EVENT_STAT;

   const Error *err = Upld32( eventStatusObj, 0, s );
   stat = (EVENT_STATUS)s;
   return err;
}
...
{
   uint32 s;
   const Error *err = Upld32( OBJID_EVENT_STAT_STICKY, 0, s );
   stat = (EVENT_STATUS)s;
   return err;
}
...
{
   uint32 s;
   const Error *err = Upld32( OBJID_EVENT_STAT_LATCH, 0, s );
   stat = (EVENT_STATUS)s;
   return err;
}
...
{
   return Dnld32(  OBJID_EVENT_STAT_LATCH, 0, (uint32)stat );
}
...
const Error *Amp::SetHomeMethod( COPLEY_HOME_METHOD method, uint16 extended )
{
   ...
   if( method == CHM_EXTENDED )
      err = Dnld16( OBJID_HOME_METHOD_EXT, 0, extended );
   else
      err = Dnld8( OBJID_HOME_METHOD, 0, (int8)method );
...
{
   uint8 i;
   const Error *err = Upld8( OBJID_HOME_METHOD, 0, i );
   method = (COPLEY_HOME_METHOD)i;
   if( err || !extended ) return err;

   return Upld16( OBJID_HOME_METHOD_EXT, 0, *extended );
}
...
{
   return Dnld32( OBJID_HOME_OFFSET, 0, PosUser2Load(value) );
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_HOME_OFFSET, 0, v ); 
   value = PosLoad2User( v );
   return err;
}
...
const Error *Amp::SetHomeVelFast( uunit value )
{
   ...
   return Dnld32( OBJID_HOME_VEL, 1, v ); 
}
...
{
   int32 v;
   const Error *err = Upld32( OBJID_HOME_VEL, 1, v );
   value = VelLoad2User( v );
   return err;
}
...
const Error *Amp::SetHomeVelSlow( uunit value )
{
   ...
   return Dnld32( OBJID_HOME_VEL, 2, v ); 
}
...
{
   int32 v;
   const Error *err = Upld32( OBJID_HOME_VEL, 2, v );
   value = VelLoad2User( v );
   return err;
}
...
{ 
   ...
   return Dnld32( OBJID_HOME_ACC, 0, v ); 
}
...
{
   int32 v;
   const Error *err = Upld32( OBJID_HOME_ACC, 0, v );
   value = AccLoad2User( v );
   return err;
}
...
{
   int32 v;
   const Error *err = Upld32( OBJID_HOME_ADJUST, 0, v );
   value = PosLoad2User( v );
   return err;
}
...
{ 
   return Dnld16( OBJID_HOME_CRNT, 0, value ); 
}
...
{
   return Upld16( OBJID_HOME_CRNT, 0, value );
}
...
{ 
   return Dnld16( OBJID_HOME_DELAY, 0, value ); 
}
...
{
   return Upld16( OBJID_HOME_DELAY, 0, value );
}
...
/**
  ...
  @param cfg The position capture configuration value is passed here.
  @return A pointer to an error object, or NULL on success
 */
const Error *Amp::SetPosCaptureCfg( POS_CAPTURE_CFG cfg )
{
   return Dnld16( OBJID_CAP_CTRL, 0, (uint16)cfg );
}

/**
  Read the current configuration of the position capture mechanism.

  @param cfg The position capture configuration value is returned here.
  @return A pointer to an error object, or NULL on success
 */
const Error *Amp::GetPosCaptureCfg( POS_CAPTURE_CFG &cfg )
{
   uint16 value;
   const Error *err = Upld16( OBJID_CAP_CTRL, 0, value );
...
/**
  ...
  @param stat The position capture status value is returned here.
  @return A pointer to an error object, or NULL on success
 */
const Error *Amp::GetPosCaptureStat( POS_CAPTURE_STAT &stat )
{
   uint16 value;
   const Error *err = Upld16( OBJID_CAP_STAT, 0, value );
...
/**
  ...
  @param value The captured position is returned here in units of encoder counts.
  @return A pointer to an error object, or NULL on success
 */
const Error *Amp::GetIndexCapture( int32 &value )
{
   return Upld32( OBJID_CAP_NDX, 0, value );
}
...
{
   return Upld32( OBJID_CAP_HOME, 0, value );
}
...
{
   return Dnld16( OBJID_PROFILE_TYPE, 0, (int16)type );
}
...
{
   int16 value;
   const Error *err = Upld16( OBJID_PROFILE_TYPE, 0, value );
...
{
   return Dnld32( OBJID_PROFILE_POS, 0, PosUser2Load(value) );
}
...
{
   int32 v;
   const Error *err = Upld32( OBJID_PROFILE_POS, 0, v ); 
   value = PosLoad2User( v );
   return err;
}
...
{
   return Dnld32( OBJID_TARGET_VEL, 0, VelUser2Load(value) );
}
...
{
   int32 v;
   const Error *err = Upld32( OBJID_TARGET_VEL, 0, v ); 
   value = VelLoad2User( v );
   return err;
}
...
{ 
   ...
   return Dnld32( OBJID_PROFILE_VEL, 0, v );
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_PROFILE_VEL, 0, v );
   value = VelLoad2User( v );
   return err;
}
...
{ 
   ...
   return Dnld32( OBJID_PROFILE_ACC, 0, v ); 
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_PROFILE_ACC, 0, v ); 
   value = AccLoad2User( v );
   return err;
}
...
{ 
   ...
   return Dnld32( OBJID_PROFILE_DEC, 0, v ); 
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_PROFILE_DEC, 0, v ); 
   value = AccLoad2User( v );
   return err;
}
...
/**
  ...
  The units used for this object are based on the CANopen DS402 specification.
  An applications note is available on Copley's web site which gives more 
  information on exactly how to convert between these torque units and the
  commanded current in the drive.
  http://www.copleycontrols.com/Motion/pdf/Current-Scaling.pdf
...
{
   return Dnld16( OBJID_TORQUE_TARGET, 0, value ); 
}
...
/**
  ...
  of the motor rated torque (see Amp::SetTorqueTarget more more info).
  @return A pointer to an error object, or NULL on success
 */
/***************************************************************************/
const Error *Amp::GetTorqueTarget( int16 &value )
{
   return Upld16( OBJID_TORQUE_TARGET, 0, value ); 
}
...
{
   return Upld16( OBJID_TORQUE_DEMAND, 0, value ); 
}
...
{
   return Upld16( OBJID_TORQUE_ACTUAL, 0, value ); 
}
...
{
   return Dnld32( OBJID_TORQUE_SLOPE, 0, value ); 
}
...
{
   return Upld32( OBJID_TORQUE_SLOPE, 0, value ); 
}
...
{
   return Dnld32( OBJID_TORQUE_RATED, 0, value ); 
}
...
{
   return Upld32( OBJID_TORQUE_RATED, 0, value ); 
}
...
{
   return Dnld16( OBJID_HALT_MODE, 0, (int16)mode );
}
...
{
   int16 m;
   const Error *err = Upld16( OBJID_HALT_MODE, 0, m );
   mode = (HALT_MODE)m;
   return err;
}
...
{
   return Dnld16( OBJID_QSTOP_MODE, 0, (int16)mode );
}
...
{
   int16 m;
   const Error *err = Upld16( OBJID_QSTOP_MODE, 0, m );
...
{ 
   return Dnld32( OBJID_PROFILE_QSTOP, 0, AccUser2Load(value) ); 
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_PROFILE_QSTOP, 0, v ); 
   value = AccLoad2User( v );
   return err;
}
...
{ 
   ...
   return Dnld32( OBJID_PROFILE_JRK, 0, v ); 
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_PROFILE_JRK, 0, v ); 
...
{
   return Upld16( OBJID_PVT_BUFF_CT, 0, n );
}
...
{
   return Upld16( OBJID_PVT_SEG_ID, 0, id ); 
}
...
{
   return Upld32( OBJID_PVT_BUFF_STAT, 0, stat ); 
}
...
{ 
   return Dnld32( OBJID_POSERR_WIN, 0, PosUser2Load(value) ); 
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_POSERR_WIN, 0, v ); 
   value = PosLoad2User( v );
   return err;
}
...
{ 
   return Dnld32( OBJID_POSWARN_WIN, 0, PosUser2Load(value) ); 
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_POSWARN_WIN, 0, v ); 
   value = PosLoad2User( v );
   return err;
}
...
{
   return Dnld32( OBJID_SETTLE_WIN, 0, PosUser2Load(value) );
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_SETTLE_WIN, 0, v ); 
   value = PosLoad2User( v );
   return err;
}
...
{ 
   return Dnld16( OBJID_SETTLE_TIME, 0, value ); 
}
...
{ 
   return Upld16( OBJID_SETTLE_TIME, 0, value ); 
}
...
{
   return Dnld32( OBJID_VELWARN_WIN, 0, VelUser2Mtr(value) ); 
}
...
{ 
   int32 v;
   const Error *err = Upld32( OBJID_VELWARN_WIN, 0, v ); 
   value = VelMtr2User( v );
   return err;
}
...
{ 
   return Dnld16( OBJID_VELWARN_TIME, 0, value ); 
}
...
{ 
   return Upld16( OBJID_VELWARN_TIME, 0, value ); 
}
...
{
   return Upld16( OBJID_STATUS, 0, value );
}
...
const Error *Amp::SetControlWord( uint16 value )
{
   ...
   // Update the value in my control pdo.  This is only used with EtherCAT and is
   // used mostly for Linkage control.  I update it here just to make sure the 
   // PDO data remains consistent with the value I'm writing via SDO
   xmitCtrlPDO( value );

   const Error *err = Dnld16( OBJID_CONTROL, 0, value );
...
{
   return Upld16( OBJID_CONTROL, 0, value );
}
...
{
   return Dnld16( OBJID_PROG_CRNT, 0, crnt );
}
...
{
   return Upld16( OBJID_PROG_CRNT, 0, crnt );
}
...
{
   return Dnld32( OBJID_PROG_VEL, 0, VelUser2Mtr(vel) ); 
}
...
{
   int32 v;
   const Error *err = Upld32( OBJID_PROG_VEL, 0, v ); 
   vel = VelMtr2User( v );
   return err;
}
...
{
   return Dnld16( OBJID_USTEP_RATE, 0, rate );
}
...
{
   return Upld16( OBJID_USTEP_RATE, 0, rate );
}
...
const Error *Amp::GetTraceStatus( AMP_TRACE_STATUS &stat, int16 &samp, int16 &sampMax )
{
   ...
   err = Upld16( OBJID_TRACE_STATUS, 0, s );
   if( !err ) err = Upld16( OBJID_TRACE_SAMP_CT,  0, samp );
   if( !err ) err = Upld16( OBJID_TRACE_SAMP_MAX, 0, sampMax );
...
{
   return Upld32( OBJID_TRACE_REF_PER, 0, per );
}
...
{
   return Upld16( OBJID_TRACE_PERIOD, 0, per );
}
...
{
   return Dnld16( OBJID_TRACE_PERIOD, 0, per );
}
...
const Error *Amp::GetTraceTrigger( AMP_TRACE_TRIGGER &type, uint8 &chan, int32 &level, int16 &delay )
{
   ...
   const Error *err = Upload( OBJID_TRACE_TRIGGER, 0, size, buff );
   if( !err ) err = Upld16( OBJID_TRACE_DELAY, 0, delay );
...
const Error *Amp::SetTraceTrigger( AMP_TRACE_TRIGGER type, uint8 chan, int32 level, int16 delay )
{
   ...
   const Error *err = Download( OBJID_TRACE_TRIGGER, 0, 6, buff );
   if( !err ) err = Dnld16( OBJID_TRACE_DELAY, 0, delay );
   return err;
}
...
{
   return Upld8( OBJID_TRACE_CHANNELS, 0, max );
}
...
const Error *Amp::GetTraceChannel( uint8 ndx, AMP_TRACE_VAR &value )
{
   ...
   uint16 v;
   const Error *err = Upld16( OBJID_TRACE_CHANNELS, ndx+1, v );
...
{
   if( ndx == 255 ) return &SDO_Error::Subindex;
   return Dnld16( OBJID_TRACE_CHANNELS, ndx+1, (uint16)value );
}
...
{
   return Dnld16( OBJID_TRACE_START, 0, (uint16)1 );
}
...
{
   return Dnld16( OBJID_TRACE_START, 0, (uint16)0 );
}
...
const Error *Amp::GetTraceData( int32 *data, int32 &max )
{
   ...
   const Error *err = Upload( OBJID_TRACE_DATA, 0, size, bptr );
   if( err ) return err;
...
// Adjust the index based on the axis number of this Amp
int16 Amp::AdjustObjIndex( int16 index )
{
   switch( index & 0xF800 )
   {
      case 0x2000:
      case 0x6000: 
         return index + 0x800*axisNum;
      default:
         return index;
   }
}

// Utility function to set the axis for input/output config methods.
// Bits 12-13 are used to set the axis
uint16 Amp::SetIOConfigAxis( uint16 config, uint16 axis )
{
   // Define a bitmask to set bits 12-13
   uint16 bitMask = 0x3000;

   // Set the axis in bits 12-13
   config = (config & (~bitMask)) | (axis<<12);

   return config;
}

// Utility function to get the axis from the input/output config.
// Bits 12-13 hold the axis number
uint16 Amp::GetIOConfigAxis( uint16 config )
{
   // Define a bitmask for bits 12-13
   uint16 bitMask = 0x3;

   // Get the axis from bits 12-13
   uint16 axis = ( config >> 12 ) & bitMask;

   return axis;

}

// this macro is used below to perform the SDO operation.
// For normal single axis nodes it just calls the corresponding SDO function.
// For multi-axis nodes it adjusts the index, locks the primary Amp object,
// and uses the primaries SDO to perform the function.
#define DO_SDO( FUNC ) \
   if( !primaryAmpRef ) return sdo.FUNC; \
index = AdjustObjIndex( index ); \
RefObjLocker<Amp> pri( primaryAmpRef ); \
if( !pri ) return &AmpError::PrimaryGone; \
return pri->sdo.FUNC;
...
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Download( int16 index, int16 sub, int32 size, byte *data )
{
   DO_SDO( Download( index, sub, size, data ) );
}
...
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Upload( int16 index, int16 sub, int32 &size, byte *data )
{
   DO_SDO( Upload( index, sub, size, data ) );
}
...
/// @param index The index of the object to be downloaded.
/// @param sub The sub-index of the object to be downloaded.
/// @param data The value to be downloaded.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Dnld32( int16 index, int16 sub, uint32 data )
{
   DO_SDO( Dnld32( index, sub, data ) );
}
...
/// @param data The value to be downloaded.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Dnld32( int16 index, int16 sub, int32 data )
{
   DO_SDO( Dnld32( index, sub, data ) );
}
...
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Upld32( int16 index, int16 sub, uint32 &data )
{
   DO_SDO( Upld32( index, sub, data ) );
}
...
/// @param data The uploaded data will be returned here.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Upld32( int16 index, int16 sub, int32 &data )
{
   DO_SDO( Upld32( index, sub, data ) );
}
...
/// @param data The value to be downloaded.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Dnld16( int16 index, int16 sub, uint16 data )
{
   DO_SDO( Dnld16( index, sub, data ) );
}
...
/// @param data The value to be downloaded.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Dnld16( int16 index, int16 sub, int16 data )
{
   DO_SDO( Dnld16( index, sub, data ) );
}
...
/// @param data The uploaded data will be returned here.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Upld16( int16 index, int16 sub, uint16 &data )
{
   DO_SDO( Upld16( index, sub, data ) );
}
...
/// @param data The uploaded data will be returned here.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Upld16( int16 index, int16 sub, int16 &data )
{
   DO_SDO( Upld16( index, sub, data ) );
}
...
/// @param data The value to be downloaded.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Dnld8( int16 index, int16 sub, uint8 data )
{
   DO_SDO( Dnld8( index, sub, data ) );
}
...
/// @param data The value to be downloaded.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Dnld8( int16 index, int16 sub, int8 data )
{
   DO_SDO( Dnld8( index, sub, data ) );
}
...
/// @param data The uploaded data will be returned here.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Upld8( int16 index, int16 sub, uint8 &data )
{
   DO_SDO( Upld8( index, sub, data ) );
}
...
/// @param data The uploaded data will be returned here.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::Upld8( int16 index, int16 sub, int8 &data )
{
   DO_SDO( Upld8( index, sub, data ) );
}
...
/// @param data The value to be downloaded.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::DnldString( int16 index, int16 sub, char *data )
{
   DO_SDO( DnldString( index, sub, data ) );
}
...
/// @param data The uploaded data will be returned here.
/// @return A pointer to an error object, or NULL on success
const Error *Amp::UpldString( int16 index, int16 sub, int32 &len, char *data )
{
   DO_SDO( UpldString( index, sub, len, data ) );
}
