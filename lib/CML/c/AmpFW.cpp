...
public:
   ...
   /**
     ...
     */
   /***************************************************************************/
   FirmwareUpdater( CanOpen *co, uint32 xmit )
   {
      coPtr = co;
      xmitId = xmit;
   }
   ...
   /**
     ...
     */
   /***************************************************************************/
   const Error *SendCmd( uint16 cmd, Timeout timeout )
   {
   ...
   /**
     ...
     */
   /***************************************************************************/
   const Error *SendCmd( uint16 cmd, uint16 ct, uint16 *param, Timeout timeout )
   {
      const Error *err;
      CanFrame frame;
...
const Error *CopleyNode::FirmwareUpdate( Firmware &fw )
{
   ...
   // For now, the firmware update only works on CANopen 
   // networks.
   if( GetNetworkType() != NET_TYPE_CANOPEN )
      return &FirmwareError::notSupported;
   ...
   uint32 xmit = 0x600 + GetNodeID();
   uint32 recv = 0x580 + GetNodeID();

   RefObjLocker<CanOpen> co( GetNetworkRef() );
   if( !co ) return &NodeError::NetworkUnavailable;

   FirmwareUpdater up( co, xmit );
   co->DisableReceiver( recv );
   err = co->EnableReceiver( recv, &up );
