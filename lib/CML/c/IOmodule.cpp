...
/**
  ...
  */
/***************************************************************************/
IOModule::IOModule( Network &net, int16 nodeID )
{
   Init( net, nodeID );
}
...
/**
  ...
  */
/***************************************************************************/
IOModule::IOModule( Network &net, int16 nodeID, IOModuleSettings &settings )
{
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::Init( Network &net, int16 nodeID )
{
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::Init( Network &net, int16 nodeID, IOModuleSettings &settings )
{
   // Init the base class
   ...
   if( ct && settings.useStandardDoutPDO )
   {
      ...
      err = doutPDO.Init( this, cobID, ct, ids );
   ...
   if( settings.useStandardAoutPDO )
   {
      ...
      for( int n=0; n<3 && ct>0; n++ )
      {
         ...
         cobID += 0x100;
         err = aoutPDO[n].Init( this, cobID, map, ids );
   ...
   if( ct && settings.useStandardDinPDO )
   {
      ...
      err = dinPDO.Init( this, cobID, ct, ids, IOEVENT_DIN_PDO0 );
   ...
   if( settings.useStandardAinPDO )
   {
      ...
      for( int n=0; n<3 && ct>0; n++ )
      {
         ...
         cobID += 0x100;
         err = ainPDO[n].Init( this, cobID, map, ids, (IOMODULE_EVENTS)(IOEVENT_AIN_PDO0<<n) );
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::WaitIOEvent( IOMODULE_EVENTS event, Timeout timeout )
{
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::WaitIOEvent( Event &e, Timeout timeout, IOMODULE_EVENTS &match )
{
...
{
   RefObjLocker<Network> net( io->GetNetworkRef() );
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::DigInPDO::Init( class IOModule *io, uint32 cobID, uint8 ct, uint8 id[], IOMODULE_EVENTS event )
{
   if( ct < 1 || ct > 8 )
      return &IOError::BadIOCount;
   ...
   const Error *err = TPDO::Init( cobID );
   if( !err ) err = SetType( 255 );
...
/**
  ...
  */
/***************************************************************************/
const Error *IOModule::AlgInPDO::Init( class IOModule *io, uint32 cobID, uint8 ct, uint8 id[], IOMODULE_EVENTS event )
{
   if( ct < 1 || ct > 4 )
      return &IOError::BadIOCount;
