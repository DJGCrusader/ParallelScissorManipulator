...
const Error *KvaserCAN::Open( void )
{
   ...
   Handle_Rd = LPcanOpenChannel( port, canWANT_EXTENDED);
   if( Handle_Rd < 0 )
      status = Handle_Rd;
   else
      status = LPcanIoCtl(Handle_Rd, canIOCTL_FLUSH_RX_BUFFER, NULL, 0 );
   ...
   if( status == canOK )
   {
      Handle_Wr = LPcanOpenChannel( port, canWANT_EXTENDED);
...
/**
  ...
  */
/***************************************************************************/
const Error *KvaserCAN::RecvFrame( CanFrame &frame, Timeout timeout )
{
   canStatus       status;
   uint    dlc, flags;
   ulong   time;
...
/**
  ...
  */
/***************************************************************************/
const Error *KvaserCAN::XmitFrame( CanFrame &frame, Timeout timeout )
{
   canStatus status;
   long id;
   uint dlc, flags;
...
static void *LoadLibrary( const char *name )
{
   ...
   if( !handle )
      cml.Warn( "Error loading Kvaser library: %s\n", dlerror() );
...
static fptr GetProcAddress( void *handle, const char *name )
{
   // NOTE - This union is a bit of a hack around an incompatibility between the POSIX standard and the 
   //        ISO C++ standard.  POSIX defines dlsym as returning a void *, ISO C++ standard makes it 
   //        illegal to convert a void* to a function pointer.
   //        There's currently no solution that's guaranteed to work on all platforms, but using the union
   //        like we're doing here should be OK on any system where the pointers can be converted without errors.
   union
   {
      void *vptr;
      fptr Fptr;
   ...
   x.vptr = dlsym( handle, name );

   if( !x.vptr )
      cml.Warn( "Error loading function %f from Kvaser shared library\n", name );
