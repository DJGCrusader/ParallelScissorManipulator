...
enum AMP_OBJID
{
   ...
   OBJID_INPUTS32              = 0x219A,
   OBJID_PULLUPS32             = 0x219C,

   OBJID_PHASE_ANG             = 0x2260,
   OBJID_HALL_STATE            = 0x2261,
   OBJID_PHASE_MODE            = 0x21C0,
   OBJID_USTEP_RATE            = 0x21C1,
   OBJID_APHASE_INIT_CUR       = 0x21C2,
   OBJID_APHASE_INIT_TIME      = 0x21C3,
   OBJID_APHASE_INIT_CONFIG    = 0x21C4,
   ...
   OBJID_VEL_OUTFILT2          = 0x210A,
   OBJID_VEL_OUTFILT3          = 0x210B,
   OBJID_CRNT_CMDFILT          = 0x210C,
   OBJID_CRNT_CMDFILT2         = 0x210D,
   
   OBJID_INSHAPE_FILT          = 0x2254,
   ...
   OBJID_MOTOR_WRAP            = 0x2220,
   OBJID_LOAD_WRAP             = 0x2221,
   OBJID_MTR_ENC_OPTIONS       = 0x2222,
   OBJID_LOAD_ENC_OPTIONS      = 0x2223,
   ...
   OBJID_PWMIN_MIN_PULSE       = 0x2323,
   OBJID_PWMIN_MAX_PULSE       = 0x2324,
   ...
   OBJID_POS_XKP               = 0x2378,
   OBJID_POS_XKI               = 0x2379,
   OBJID_POS_XKD               = 0x237A,
   OBJID_POS_KI_DRAIN          = 0x2382,

   OBJID_AMP_NAME              = 0x21A0,

   OBJID_CANID_CFG             = 0x21B0,
   OBJID_CANID_PIN_MAP         = 0x21B1,

   OBJID_CAN_NETWORK_OPTIONS   = 0x21B3,
   ...
   OBJID_USTEP_DETENT_COR_GAIN = 0x21D2,
   OBJID_USTEP_VOLTIME         = 0x21D5,
   OBJID_USTEP_CONFIG_STAT     = 0x21D6,
   OBJID_USTEP_P_GAIN_OUT_LOOP = 0x21D7,
   OBJID_USTEP_MAX_VEL_ADJ     = 0x21D8,
   ...
   OBJID_GAIN_SCH_CONFIG       = 0x2370,
   OBJID_GAIN_SCH_KEY          = 0x2371,
   ...
   OBJID_CAN_QSTOP             = 0x605A,
   OBJID_CAN_SHUT_DOWN         = 0x605B,
   OBJID_CAN_DISABLE           = 0x605C,
   OBJID_CAN_HALT              = 0x605D,

   OBJID_CAN_HARTBEAT_TIME     = 0x1017,
   OBJID_CAN_NODEGUARD_TIME    = 0x100C,
   OBJID_CAN_NODEGUARD_LIFE    = 0x100d,
   ...
   OBJID_AIN_FILT              = 0x2109,

   OBJID_AOUT_CFG              = 0x21E0,
   OBJID_IO_OPTIONS            = 0x2198,

   OBJID_SERVO_LOOP_CFG        = 0x2301,

   OBJID_MTR_TEMP_LIMIT        = 0x220A,
   OBJID_BRAKE_ENABLE_DELAY    = 0x2199,
   OBJID_OPEN_MTR_CRNT         = 0x2142,

   OBJID_LDENC_CAL             = 0x2227,
   OBJID_MTRENC_CAL            = 0x2226,

   OBJID_ENCCOS_SCALE          = 0x220D,
   OBJID_ENCCOS_OFFSET         = 0x220C,
   OBJID_ENCSIN_OFFSET         = 0x220B,

   OBJID_PWMIN_UVCFG           = 0x2326
};
...
Note: Bits 12-13 of the input config specify the axis the input configuration
will be applied to. 
...
enum INPUT_PIN_CONFIG
{
   ...
   INCFG_ENCODER_FAULT_H       = 0x001C, ///< Encoder fault input, active high
   INCFG_ENCODER_FAULT_L       = 0x001D, ///< Encoder fault input, active low
   ...
   INCFG_HV_LOSS_DISABLE_H     = 0x0026, ///< Mark HV loss on rising edge, disable while high.
   INCFG_HV_LOSS_DISABLE_L     = 0x0027, ///< Mark HV loss on falling edge, disable while low.
   INCFG_TRJ_UPDATE_R          = 0x0028, ///< Trajectory update on rising edge.
   INCFG_TRJ_UPDATE_F          = 0x0029, ///< Trajectory update on falling edge.
   INCFG_CLR_FAULTS_EVENTS_R   = 0x002A, ///< Clear faults and event latch on rising edge.
   INCFG_CLR_FAULTS_EVENTS_F   = 0x002B, ///< Clear faults and event latch on falling edge.
   INCFG_DIS_SIM_ENC_L_BURST_R = 0x002C, ///< Disable simulated encoder output when low. Burst current position on encoder output on rising edge.
   INCFG_DIS_SIM_ENC_H_BURST_F = 0x002D  ///< Disable simulated encoder output when high. Burst current position on encoder output on falling edge. 
};
...
enum EVENT_STATUS
{
   ...
   ESTAT_CMD_INPUT     = 0x40000000   ///< Command input fault is active
};
...
enum PROFILE_TYPE
{
   ...
   /// Cyclic Synchronous Position (CSP) profile.
   PROFILE_CSP = 4
};
...
enum AMP_PHASE_MODE
{
   ...
   /// Use analog hall inputs offset at 90 deg connected to the encoder sine / cosine
   /// inputs.
   ...
   /// NOTE - this mode is obsolete.  The motor type parameter (object 0x2383)
   /// should now be used to indicate a brush motor which will force the 
   /// correct commutation mode.
   ...
   /// Use phase information obtained from the encoder.  This mode is used with resolvers
   /// and most absolute encoders which are able to communicate phasing information along
   /// with the position.
   PHASE_MODE_ENCPHASE    = 6,

   /// Use interpolated trapezoidal commutation.  This mode can be used when digital hall 
   /// sensors are available on the motor.  At high speeds the amp interpolates between the 
   /// hall sensor angles and creates a smoother commutation angle.
...
/***************************************************************************/
/**
  This enumeration is used internally by the amplifier object to check for 
  certain features that are not present in every amp model or firmware version
  number.
 */
/***************************************************************************/
enum AMP_FEATURE
{
   // Can the amplifier support voltage loop drain for integral sum?
   FEATURE_VI_DRAIN,

   /// Can the amplifier firmware accept more then 6 words of data for it's output pin configuration parameter?
   FEATURE_EXTENDED_OUTPUT_PIN_CONFIG,

   /// Does the amplifier support the gear ratio parameter?
   FEATURE_GEAR_RATIO,

   /// Does the amplifier support the resolver cycles parameter?
   FEATURE_RESOLVER_CYCLES,

   /// Does the amplifier support the hall velocity shift parameter?
   FEATURE_HALL_VEL_SHIFT,

   /// Does the amplifier support the position loop scaling factor?
   FEATURE_PLOOP_SCALE,

   /// Does the amp support the stepper current parameters (hold current, run to hold time, etc)?
   FEATURE_STEPPER_CRNT,

   /// Does the amp support the current slope limits?
   FEATURE_CURRENT_SLOPE,

   /// Does the amp support the software limit acceleration parameter?
   FEATURE_SOFTLIM_ACCEL,

   /// Does the amp support the pwm input frequency parameter?
   FEATURE_PWMIN_FREQ,

   /// Does the amp support the velocity loop command filter?
   FEATURE_VLOOP_CMD_FILT,

   /// Does the amp support ustep outer loop
   FEATURE_USTEP_OUTER_LOOP,

   /// Does the amp support ustep Detent gain correction factor
   FEATURE_STEP_DETENT_GAIN,

   /// Does the amp support ustep Stepper config and status
   FEATURE_USTEP_CONFIG_STATUS,

   /// Does the amp support algorithmic phase init max current an time settings
   FEATURE_ALGO_PHASE_INIT_CUR,

   /// Does the amp support algorithmic phase init config
   FEATURE_ALGO_PHASE_INIT_CONFIG,

   /// Does the amp support camming
   FEATURE_CAMMING,

   /// Does the amp support positon wrap
   FEATURE_POS_WRAP,

   /// Does the amp support encoder options
   FEATURE_ENC_OPTIONS,

   /// Does the amp support gain scheduling
   FEATURE_GAIN_SCHED,

   /// Does the amp support input pin mapping
   FEATURE_PIN_MAP,

   /// Does the amp support can option 
   FEATURE_CAN_OPTIONS,

   /// Does the amp support can option
   FEATURE_CAN_SETTINGS,

   /// Does the amp support analog reference input filter.
   FEATURE_AIN_FILT,

   /// Does the amp support the velocity loop cmd filter
   FEATURE_VLOOP_OUT_FILT,

   /// Does the amp support the current loop cmd filter
   FEATURE_ILOOP_CMD_FILT,

   /// Does the amp support configuring the min pwm pulse width
   FEATURE_PWMIN_MIN_PULSE,

   /// Does the amp support configuring the max pwm pulse width
   FEATURE_PWMIN_MAX_PULSE,

   /// Does the amp support DA configuration
   FEATURE_DA_CONV_CONFIG,

   /// Does the amp support advanced servo loop configurations
   FEATURE_SERVO_CONFIG,

   /// Does the amp support analog mtr over temp
   FEATURE_MTR_OVERTEMP,

   /// Does the amp support configuring the network
   FEATURE_NET_OPTIONS,

   /// Does the amp support the position loop Ki param
   FEATURE_PLOOP_KI,

   /// Does the amp support the position loop Kd param
   FEATURE_PLOOP_KD,

   /// Does the amp support the velocity loop command feed forward
   FEATURE_VLOOP_CMDFF,

   /// Does the amp support motor brake enable delay time
   FEATURE_BRAKE_ENABLE_DELAY,

   /// Does the amp support input shaping
   FEATURE_INPUT_SHAPING,

   /// Are filter coefficients stored at floating point values?
   FEATURE_FLOAT_FILT_COEF,
   
   /// Does the amp support IO configuration
   FEATURE_IO_OPTIONS,

   /// Does the amp support the UV config in UV mode?
   FEATURE_PWMIN_UVCFG,

   /// Does the amp support the paramter containing the number of axis?
   FEATURE_AXIS_CT,

   // Does the amp support encoder calibration params
   FEATURE_ENC_CAL,

   // Does the amp support encoder calibration params
   FEATURE_MTR_OPEN_CRNT,

   // Does the amp support cross coupling
   FEATURE_CROSS_COUPLE,

   // Does the amp support position loop ki drain
   FEATURE_POS_KI_DRAIN,

   // iopullup32 was set in fw as read only until 2.22 firmware
   FEATURE_IOPULLUP32
};
