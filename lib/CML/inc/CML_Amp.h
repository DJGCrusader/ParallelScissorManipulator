...
public:
   ...
   static const AmpError PhaseInit;     ///< Amplifier is currently performing a phase initialization.
   static const AmpError NotHoming;     ///< The amplifier is not currently configured for homing mode.
   static const AmpError HomingError;   ///< The amplifier did not complete the homing method successfully.
   static const AmpError BadAxis;       ///< Illegal axis number specified
   static const AmpError PrimaryGone;   ///< Primary axis Amp object no longer available
   static const AmpError NotInit;       ///< Amp object not properly initialized
...
public:
   ...
   ///
   /// For EtherCAT networks, this parameter specifies the process data heartbeat 
   /// timeout.  This is similar to the CANopen node guarding timeout.  If the drive
   /// is operational, and goes this long without receiving process data from the master,
   ...
   ///
   /// Note that resetting nodes on an EtherCAT network will make it impossible
   /// to access anything on the network while the reset is occuring.  Resetting
   /// EtherCAT nodes is therefore not recommended.
   ///
   /// Default: false
   bool resetOnInit;
...
public:
   ...
   */
   ...
   /***************************************************************************/
   /** \name Low level access to CANopen objects.
   */
   /***************************************************************************/
   //@{
   ...
   */
   ...
   /// WARNING - this function is dangerous as the pointer is not locked.
   ///           Use Amp::GetLinkRef instead!
   ...
   /// @return The linkage reference, or 0 if the Amp is not attached to
   ///         any linkage object.
...
private:
   const Error *InitAxis( AmpSettings &settings );
   const Error *SetupSynchPDO( AmpSettings &settings );
   int16 AdjustObjIndex( int16 index );
   uint16 SetIOConfigAxis( uint16 config, uint16 axis );
   uint16 GetIOConfigAxis( uint16 config );
   void FinishPvtTrj( void );

   int CheckFeature( AMP_FEATURE feature );

   /// Axis number for multi-axis nodes
   int axisNum;

   /// For multi-axis nodes, we keep a reference to the primary Amp object
   uint32 primaryAmpRef;
   ...
   // Max PVT segments that can be written to the buffer at once
   uint16 pvtMaxSegWrite;
   ...
   /// Reference to the currently running PVT trajectory
   uint32 pvtTrjRef;
   ...
   const Error *InitPDOs( void );
   const Error *UninitPDOs( void );
   void         FreePDOs( void );

   // Status PDO transmitted by drive
   class TPDO_Status *statPDO;
   EVENT_STATUS getPdoEventStat();
   uint16       getPdoDS402Stat();

   // Control PDO used on EtherCAT networks
   class RPDO_Ctrl *ctrlPDO;
   const Error *xmitCtrlPDO( uint16 val );

   /// A transmit PDO used to send PVT buffer count information.
   class TPDO_PvtStat *pvtStatPDO;

   /// A receive PDO used to transmit PVT segment 
   /// information to the amplifier
   class RPDO_PvtCtrl *pvtCtrlPDO;
   const Error *xmitPvtPDO( uint8 val[] );
   ...
   void SetLinkage( class Linkage *l );
   uint32 linkRef;
