 ...
 * This pure virtual class is the base for an OS specific 
 * class which implements the interface to the Ethernet hardware.
 */
...
private:
   ...
   bool LoadPdoDat( uint32 ref, uint8 *buff, int max );
...
/** Generic EtherCAT datagram class.
   ...
   In general, CML uses don't need to concern themselves with EtherCAT datagrams.  This 
   class is used internally in the libary to support low level communications over the
   EtherCAT network.
*/
class EcatDgram
{
