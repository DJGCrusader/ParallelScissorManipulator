...
public:
   ...
   /// If true, the master (i.e. the computer that CML is running on) will
   /// generate sync timing messages used to synchronize the clocks of the 
   /// nodes on the CANopen network.  This is only possible if the CAN 
   /// interface being used is able to capture accurate PC timer based 
   /// time stamps of the received CAN frames.  Since this isn't supported
   /// by most CAN interfaces, the default setting for this is false.
   /// If false, one of the nodes on the CANopen network will generate the
   /// time stamps.
   /// Default: false
   bool useAsTimingReference;

   /// CAN message ID of the SYNC message.  This is only used if the CanOpen
   /// object wll act as the timing reference in the system.  If so, this ID
   /// must match the corresponding value passed in the AmpSettings object.
   /// Default 0x80
   uint32 syncID;

   /// CAN message ID of the timestamp message.  This is only used if the CanOpen
   /// object wll act as the timing reference in the system.  If so, this ID
   /// must match the corresponding value passed in the AmpSettings object.
   /// Default 0x180
   uint32 timeID;
...
private:
   ...
   /// Is CML acting as the master time keeper in the system?
   bool timingMaster;

   /// Class used to generate timestamps 
   class TimeStampGenerator *tsGen;
