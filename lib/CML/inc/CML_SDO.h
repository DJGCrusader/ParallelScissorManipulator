...
#define SDO_ABORT_OBJ_MAP_ACTIVE         0x06010003
...
protected:
   ...
   SDO_Error( uint16 id, const char *desc ): Error( id, desc ){}
};
...
public:
   ...
   void SetTimeout( Timeout to ){ timeout = to; }

   /// Get the timeout used with this SDO
   /// @return The timeout in milliseconds
   Timeout GetTimeout( void ){ return timeout; }
   ...
   /// Return the maximum number times that the SDO transfer will
   /// be attempted before returning an error.
   /// @return The max number of retries
   uint8 GetMaxRetry( void ){ return maxRetry; }

   /// Set the maximum number of times the SDO transfer will be 
   /// attempted before returning an error
   void SetMaxRetry( uint8 max );

private:
   const Error *XmitSDO( uint8 *buff, uint16 len, uint16 *retLen, Timeout timeout );
   const Error *UploadFinish( int32 &size, byte *data, uint16 retLen, uint8 *buff );

   const Error *SendAbort( const Error *err );
   const Error *SendAbort( int32 abortCode, const Error *err );
   const Error *getAbortRcvdErr( uint8 *buff );
   const Error *_Download( int16 index, int16 sub, int32 size, byte *data );
   const Error *_Upload( int16 index, int16 sub, int32 &size, byte *data );

   /// Node the SDO belongs to
   Node *node;
   ...
   /// Maximum number of times to retry the transfer before returning an error.
   uint8 maxRetry;
};
